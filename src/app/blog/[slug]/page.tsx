...
<section className="pb-20 px-6">
  <div className="max-w-7xl mx-auto">
    {loading ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 justify-items-center auto-rows-fr">
        {filteredPosts.map((post, index) => (
          ...
          >
            <article className="h-[550px] w-full bg-white/5 backdrop-blur-sm rounded-3xl overflow-hidden border border-white/10 hover:border-purple-500/50 transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl hover:shadow-purple-500/10 flex flex-col">
              ...
              <div className="relative h-[200px] w-full flex-shrink-0 overflow-hidden">
                ...
                {/* Date badge */}
                <div className="absolute top-3 right-3 px-2 py-1 bg-black/60 backdrop-blur-sm rounded-full text-xs text-white border border-white/20">
                  {new Date(post.created_at).toLocaleDateString('en-US', {
                    month: 'short',
                    day: 'numeric'
                  })}
              ...
              {/* Content section - Remaining height (350px) */}
              <div className="h-[350px] p-6 flex flex-col">
                {/* Tags - Fixed space */}
                <div className="flex flex-wrap gap-1 mb-3 h-[32px] overflow-hidden">
                  {post.tags.split(",").slice(0, 2).map((tag: string) => (
                    <span
                      key={tag}
                      className="inline-block text-xs px-2 py-1 bg-gradient-to-r from-purple-600/20 to-pink-600/20 text-purple-200 rounded-full border border-purple-500/30"
                    >
                      {tag.trim()}
                    </span>
                  ))}
                ...
                <h2 className="text-lg font-bold mb-3 text-white group-hover:text-purple-200 transition-colors duration-300 leading-tight h-[60px] overflow-hidden">
                  {post.title}
                </h2>

                {/* Excerpt - Takes remaining space */}
                <div className="flex-1 mb-4">
                  <p className="text-gray-400 leading-relaxed text-sm overflow-hidden">
                    {post.content.replace(/[#*`]/g, '').slice(0, 100)}...
                  </p>
                </div>

                {/* Read more - Fixed at bottom */}
                <div className="flex items-center text-purple-400 text-sm font-medium opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 mt-auto">
